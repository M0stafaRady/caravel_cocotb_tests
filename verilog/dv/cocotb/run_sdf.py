...
if __name__ == "__main__":
    test_list = "regression/all_sdf.yaml"
    sdf_types = ["low", "high"]
    corners = ["nom-t", "min-f", "max-s", "min-t", "max-t", "max-f", "nom-f", "nom-s", "min-s"]
    # corners = ["nom-s", "min-s", "max-s"]
    commands = []
    for sdf_type in sdf_types:
        for corner in corners:
            command = f"caravel_cocotb -tl {test_list} -sim GL_SDF -vcs  -no_wave -tag run_sdf_{sdf_type}_{corner}_25ns_{datetime.now().strftime('%d_%b_%H_%M_%S_%f')[:-4]}  -corner {corner} -verbosity quiet -no_gen_defaults"
            if sdf_type == "high":
                command += " -sdf_setup "
